...
const TopBar = () => {
  ...
  useEffect(() => {
    fetch('https://watch-shops.herokuapp.com/admin?email=' + user.email)
      .then(res => res.json())
      ...
      <div class="container">
        <ul class="navbar-nav d-flex justify-content-between">
          {<li className="nav-item me-5">
            ...
            {
              location.pathname === '/dashboard/add-orders' && <Link className="nav-link mt-1 page-title" to="/dashboard/add-orders"><b>Add Product</b></Link>
            }
            {
            ...
            {
              location.pathname === '/dashboard/manage-orders' && <Link className="nav-link mt-1 page-title" to="/dashboard/manage-orders"><b>Manage Product</b></Link>
            }
            {
            ...
            {
              location.pathname === '/dashboard/orders-list' && <Link className="nav-link mt-1 page-title" to="/dashboard/orders-list"><b>Orders List</b></Link>
            }
          </li>}
          {user?.email && <p className="mt-3 text-danger text-uppercase"><b>{user.displayName}</b></p>}
        </ul>
      </div>
    </nav>
  );
