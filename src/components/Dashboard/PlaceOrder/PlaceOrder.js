...
const PlaceOrder = () => {
  ...
  const handlePayment = paymentId => {
    ...
    fetch('https://watch-shops.herokuapp.com/addOrder', {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    const url = `https://watch-shops.herokuapp.com/service/${id}`
    fetch(url)
      .then(res => res.json())
  ...
  return (
    <div className="place-order mt-5">
      <div className="col-sm-6 my-4  col-12">
        <input type="text" className='form-control' defaultValue={user.displayName} placeholder='Your Name' />
      </div>
      <div className="col-sm-6 my-4  col-12">
        <input type="email" className='form-control' defaultValue={user.email} placeholder='Enter Email' />
      </div>
      <div className="col-sm-6  my-4 col-12">
